...
public class Facebook {
	...
	private void startDialogAuth(Activity activity, String[] permissions) {
		...
		dialog(activity, LOGIN, params, new DialogListener() {
			...
			public void onComplete(Bundle values) {
				...
				setAccessToken(values.getString(AppConstant.TOKEN));
				setAccessExpiresIn(values.getString(AppConstant.EXPIRES));
				if (isSessionValid()) {
					FbUtil.logd("Facebook-authorize",
							"Login Success! access_token=" + getAccessToken()
									+ " expires=" + getAccessExpires());
					mAuthDialogListener.onComplete(values);
				} else {
					mAuthDialogListener.onFacebookError(new FbError(
							"Failed to receive access token."));
				}
			}

			public void onError(FbDialogError error) {
				FbUtil.logd("Facebook-authorize", "Login failed: " + error);
			...
			public void onCancel() {
				FbUtil.logd("Facebook-authorize", "Login canceled");
	...
	public void authorizeCallback(int requestCode, int resultCode, Intent data) {
		if (requestCode == mAuthActivityCode) {
			...
			if (resultCode == Activity.RESULT_OK) {
				...
				if (error != null) {
					if (error.equals(AppConstant.SINGLE_SIGN_ON_DISABLED)
					...
					setAccessToken(data.getStringExtra(AppConstant.TOKEN));
					setAccessExpiresIn(data.getStringExtra(AppConstant.EXPIRES));
					if (isSessionValid()) {
				...
				if (data != null) {
					mAuthDialogListener.onError(new FbDialogError(data
	...
	private class TokenRefreshServiceConnection implements ServiceConnection {
		...
		final Messenger messageReceiver = new Messenger(new Handler() {
			...
			public void handleMessage(Message msg) {
				String token = msg.getData().getString(AppConstant.TOKEN);
				long expiresAt = msg.getData().getLong(AppConstant.EXPIRES) * 1000L;
				...
				resultBundle.putLong(AppConstant.EXPIRES, expiresAt);
				...
				if (token != null) {
					...
					if (msg.getData().containsKey("error_code")) {
						...
						serviceListener.onFacebookError(new FbError(
		...
		private void refreshToken() {
			...
			requestData.putString(AppConstant.TOKEN, mAccessToken);
	...
	public String request(String graphPath, Bundle params, String httpMethod)
		...
		if (isSessionValid()) {
			params.putString(AppConstant.TOKEN, getAccessToken());
		}
		String url = (graphPath != null) ? AppConstant.GRAPH_BASE_URL
				+ graphPath : AppConstant.RESTSERVER_URL;
	...
	public void dialog(Context context, String action, Bundle parameters,
		...
		String endpoint = AppConstant.DIALOG_BASE_URL + action;
		parameters.putString("display", "touch");
		parameters.putString("redirect_uri", AppConstant.REDIRECT_URI);
		...
		if (isSessionValid()) {
			parameters.putString(AppConstant.TOKEN, getAccessToken());
		}
		String url = endpoint + "?" + FbUtil.encodeUrl(parameters);
		if (context.checkCallingOrSelfPermission(Manifest.permission.INTERNET) != PackageManager.PERMISSION_GRANTED) {
			FbUtil.showAlert(context, "Error",
	...
	public static interface DialogListener {
		 ...
		 * 
		 */
		public void onFacebookError(FbError e);
